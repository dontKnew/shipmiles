    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <a href="<?= route_to("admin/service/add") ?>" class="btn btn-info">
                    <i class="la la-plus-circle"></i> Add Service
                </a>
            </div>
            ...
            <div class="card-body">
                ...
                <?php endif; ?>
                <?php if ($data): ?>
                    <form action="<?= route_to("admin/service") ?>" method="post" >
                        ...
                        <table class="table table-head-bg-primary table-striped table-hover text-center">
                            ...
                            <?php $i=1; foreach ($data as $value): ?>
                                <tr>
                                    ...
                                    <td><?= esc($value['fuel_surcharge']) ?></td>
                                    <td><?= esc($value['discount']) ?></td>
                                    <td><img src="<?= base_url()."/backend/img/service/".esc($value['logo']) ?>" alt="logo network" width="40" height="40"> </td>
                                    <td>
                                        <a onclick="return confirm('Are you sure want to remove service ?')" href="<?= route_to("admin/service/delete", $value['id']) ?>"
                                           class="btn btn-sm btn btn-rounded btn-danger my-1">
                                            <i class="la la-trash"></i> Delete
                                        </a>
                                        <a href="<?= route_to("admin/service/update" , $value['id']) ?>"
